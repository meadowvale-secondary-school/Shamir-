...
func calculateTripVote(votes: [ClassTripDestination] ) -> String {
    ...
    return "The class votes for the beach were \(beachVotes) and for the choclate factory were \(factoryVotes)."
}

let voteResults = calculateTripVote(votes: tripDestinationVotes)
/*:
 ### Extension:
...
func calculateVotesMascot (votes: [SchoolMascotOption] ) -> String {
    ...
    return "The vote results for salmander were \(salmanderCount), for marmot were \(marmotCount) and for neither were \(neitherCount)."
}

let voteResult = calculateVotesMascot(votes: mascotVotes)
